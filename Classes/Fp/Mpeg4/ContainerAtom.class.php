...
abstract class Fp_Mpeg4_ContainerAtom extends Fp_Mpeg4_Atom implements Iterator, ArrayAccess
{
    ...
    public function addChild( $childType )
    {
        if( !$this->_allowAnyChildrenType && !$this->validChildType( $childType ) ) {
            ...
            throw new Fp_Mpeg4_Exception( 'Atom of type ' . $childType . ' cannot be contained in ' . $this->_type, Fp_Mpeg4_Exception::EXCEPTION_INVALID_ATOM );
        }
        
        $className         = 'Fp_Mpeg4_Atom_' . ucfirst( $childType );
        
        if( !class_exists( $className ) ) {
            
            $atom = new Fp_Mpeg4_UnknownAtom( $childType );
